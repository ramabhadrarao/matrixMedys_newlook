...
const InvoiceReceivingDetails: React.FC = () => {
  ...
  const handlePrint = async () => {
    try {
      // Use the new PDF download API instead of browser print
      await invoiceReceivingAPI.downloadPDF(id!);
  ...
  return (
    <div className="space-y-6 print-container">
      ...
      <div className="print-only print-header">
        <div className="print-title">INVOICE RECEIVING DOCUMENT</div>
        <div className="print-subtitle">#{receiving.invoiceNumber}</div>
        <div className="print-info-row">
          <span className="print-info-label">Generated on:</span>
          <span className="print-info-value">{new Date().toLocaleDateString()}</span>
        </div>
        <div className="print-info-row">
          <span className="print-info-label">Generated by:</span>
          <span className="print-info-value">{user?.name || 'System'}</span>
        </div>
      </div>

      {/* Header - Hidden in print */}
      <div className="flex justify-between items-start screen-only">
      ...
      <div className="border-b border-gray-200 screen-only">
      ...
      {/* Tab Content */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 tab-content">
        {/* Details Tab */}
        {(activeTab === 'details' || document.body.classList.contains('printing')) && (
          <div className={`p-6 ${activeTab === 'details' ? 'active-tab-content' : ''}`}>
            {/* Print section title */}
            <div className="print-only print-section-title">INVOICE RECEIVING DETAILS</div>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print-info-grid">
              {/* Invoice Information */}
              <div className="print-info-section">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center print-info-title">
                  <FileText className="w-5 h-5 mr-2 screen-only" />
                  Invoice Information
                </h3>
                
                <div className="space-y-3">
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Invoice Number:</span>
                    <span className="font-medium print-info-value">{receiving.invoiceNumber}</span>
                  </div>
                  
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Invoice Date:</span>
                    <span className="font-medium print-info-value print-date">
                      {receiving.invoiceDate ? formatDate(receiving.invoiceDate) : 'N/A'}
                    </span>
                  </div>
                  
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Invoice Amount:</span>
                    <span className="font-medium print-info-value print-currency">
                      {receiving.invoiceAmount ? formatCurrency(receiving.invoiceAmount) : 'N/A'}
                    </span>
                  </div>
                  
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Received Date:</span>
                    <span className="font-medium print-info-value print-date">{formatDate(receiving.receivedDate)}</span>
                  </div>
                  
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Supplier:</span>
                    <span className="font-medium print-info-value">{receiving.supplier}</span>
                  </div>
                  
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">QC Required:</span>
                    <span className="font-medium print-info-value">
                      {receiving.qcRequired ? 'Yes' : 'No'}
                    </span>
                  </div>

                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Status:</span>
                    <span className={`font-medium print-info-value print-status-badge status-${receiving.status.replace('_', '-')}`}>
              ...
              <div className="print-info-section">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center print-info-title">
                  <Package className="w-5 h-5 mr-2 screen-only" />
                  Purchase Order
                </h3>
                
                <div className="space-y-3">
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">PO Number:</span>
                    <span className="font-medium print-info-value">
                  ...
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">PO Status:</span>
                    <span className={`font-medium print-info-value print-status-badge ${
                      typeof receiving.purchaseOrder === 'object' && receiving.purchaseOrder.status
                        ? `status-${receiving.purchaseOrder.status.replace('_', '-')}`
                        : 'status-unknown'
                    }`}>
                      {typeof receiving.purchaseOrder === 'object' && receiving.purchaseOrder.status
                  ...
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Total Products:</span>
                    <span className="font-medium print-info-value">{receiving.receivedProducts?.length || 0}</span>
                  </div>
                  
                  <div className="flex justify-between print-info-row">
                    <span className="text-gray-600 print-info-label">Received By:</span>
                    <span className="font-medium print-info-value">{receiving.receivedBy?.name || 'N/A'}</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="mt-4 inline-flex items-center text-blue-600 hover:text-blue-800 text-sm screen-only"
            ...
            {/* Notes */}
            {receiving.notes && (
              <div className="mt-8 print-notes-section">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center print-section-title">
                  <MessageSquare className="w-5 h-5 mr-2 screen-only" />
                  Notes
                </h3>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-gray-700 print-notes-content">{receiving.notes}</p>
                </div>
              </div>
            )}
          </div>
        ...
        {/* Products Tab */}
        {(activeTab === 'products' || document.body.classList.contains('printing')) && (
          <div className={`p-6 print-products-section ${activeTab === 'products' ? 'active-tab-content' : ''}`}>
            <h3 className="text-lg font-semibold text-gray-900 mb-4 print-section-title">Received Products</h3>
              ...
              <table className="w-full print-table">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider screen-only">
                      Images
                    </th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      Remarks
                  ...
                  {receiving.receivedProducts?.map((product, index) => (
                    <tr key={index}>
                      <td className="px-4 py-4">
                        ...
                        <div className="text-sm text-gray-500 print-currency">₹{product.unitPrice}</div>
                      </td>
                      ...
                      <td className="px-4 py-4 screen-only">
        ...
        {/* QC Tab */}
        {(activeTab === 'qc' || document.body.classList.contains('printing')) && (
          <div className={`p-6 ${activeTab === 'qc' ? 'active-tab-content' : ''}`}>
            ...
            {receiving.qcRequired ? (
              <div className="space-y-4">
                {receiving.receivedProducts?.map((product, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4 page-break-inside-avoid">
        ...
        {/* Documents Tab */}
        {(activeTab === 'documents' || document.body.classList.contains('printing')) && (
          <div className={`p-6 print-documents-section ${activeTab === 'documents' ? 'active-tab-content' : ''}`}>
            <h3 className="text-lg font-semibold text-gray-900 mb-4 print-section-title">Supporting Documents</h3>
            
            {receiving.documents && receiving.documents.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 print-document-list">
                {receiving.documents.map((doc, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow print-document-item">
                    <div className="flex items-center space-x-3">
                      <FileText className="w-8 h-8 text-blue-600 screen-only" />
                    ...
                    <div className="mt-3 flex space-x-2 screen-only">
                      <button
                        onClick={() => {
                          try {
                    ...
                    {/* Print-only document info */}
                    <div className="print-only mt-2 text-xs">
                      <div>Filename: {doc.filename}</div>
                      <div>Type: {doc.originalName?.split('.').pop()?.toUpperCase() || 'Unknown'}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="print-only print-footer">
        <div>This document was generated from MatrixMedys Invoice Receiving System</div>
        <div>Page printed on {new Date().toLocaleString()}</div>
      </div>

      {/* QC Modal - Hidden in print */}
      {qcModalOpen && selectedProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 screen-only">
